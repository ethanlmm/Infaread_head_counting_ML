...
def MCNN():
    ...
    conv_m = keras.layers.Conv2D(20, 5, padding='same', dilation_rate=1,activation='relu')(inputs)
    conv_m = keras.layers.Conv2D(40, 5, padding='same', dilation_rate=1, activation='relu')(conv_m)
    conv_m = keras.layers.MaxPooling2D(pool_size=(2, 2))(conv_m)
    conv_m = keras.layers.Conv2D(80, 5, padding='same',dilation_rate=1, activation='relu')(conv_m)
    conv_m = keras.layers.Conv2D(40, 5, padding='same', dilation_rate=1, activation='relu')(conv_m)
    conv_m = keras.layers.MaxPooling2D(pool_size=(2, 2))(conv_m)
    conv_m = keras.layers.Conv2D(20, 5, padding='same',dilation_rate=1, activation='relu')(conv_m)
    conv_m = keras.layers.Conv2D(10, 5, padding='same',dilation_rate=1,  activation='relu')(conv_m)

    conv_s = keras.layers.Conv2D(24, 3, padding='same', dilation_rate=1,activation='relu')(inputs)
    conv_s = keras.layers.Conv2D(48, 3, padding='same', dilation_rate=1, activation='relu')(conv_s)
    conv_s = keras.layers.MaxPooling2D(pool_size=(2, 2))(conv_s)
    conv_s = keras.layers.Conv2D(96, 3, padding='same', dilation_rate=1, activation='relu')(conv_s)
    conv_s = keras.layers.Conv2D(48, 3, padding='same', dilation_rate=1, activation='relu')(conv_s)
    conv_s = keras.layers.MaxPooling2D(pool_size=(2, 2))(conv_s)
    conv_s = keras.layers.Conv2D(24, 3, padding='same', dilation_rate=1, activation='relu')(conv_s)
    conv_s = keras.layers.Conv2D(12, 3, padding='same', dilation_rate=1, activation='relu')(conv_s)

    conv_l = keras.layers.Conv2D(16, 7, padding='same',  dilation_rate=1,activation='relu')(inputs)
    conv_l = keras.layers.Conv2D(32, 7, padding='same',  dilation_rate=1,activation='relu')(conv_l)
    conv_l = keras.layers.MaxPooling2D(pool_size=(2, 2))(conv_l)
    conv_l = keras.layers.Conv2D(64, 7, padding='same',  dilation_rate=1,activation='relu')(conv_l)
    conv_l = keras.layers.Conv2D(32, 7, padding='same',  dilation_rate=1,activation='relu')(conv_l)
    conv_l = keras.layers.MaxPooling2D(pool_size=(2, 2))(conv_l)
    conv_l = keras.layers.Conv2D(16, 7, padding='same',  dilation_rate=1,activation='relu')(conv_l)
    conv_l = keras.layers.Conv2D(8,  7, padding='same',  dilation_rate=1, activation='relu')(conv_l)
...
def CSR_Net():
    ...
    concat = keras.layers.Concatenate(axis=3)([block1,block2,block3,block4])
...
def lmm():
    inputs = keras.layers.Input(shape=(None, None, 1))
    block1 = keras.layers.Conv2D(32, 5, padding='same', dilation_rate=1, activation='relu')(inputs)
    block1 =keras.layers.Conv2D(64, 5, padding='same', dilation_rate=1, activation='relu')(block1)
    block2 = keras.layers.Conv2D(128, 5, padding='same', dilation_rate=1, activation='relu')(block1)
    #block2 = keras.layers.Conv2D(128, 3, padding='same', dilation_rate=1, activation='relu')(block2)

    #block3 = keras.layers.Conv2D(256, 3, padding='same', dilation_rate=2, activation='relu')(block2)
    #block3 = keras.layers.Conv2D(256, 3, padding='same', dilation_rate=5, activation='relu')(block3)

    # block4 = keras.layers.Conv2D(512, 3, padding='same', dilation_rate=5, activation='relu')(block3)
    # block4 = keras.layers.Conv2D(512, 3, padding='same', dilation_rate=1, activation='relu')(block4)

    #concat = keras.layers.Concatenate(axis=3)([block1, block2, block3])
    end = keras.layers.MaxPooling2D(pool_size=(2, 2))(block2)
    ...
    end = keras.layers.Conv2D(32, 3, padding='same', dilation_rate=1, activation='relu')(end)
...
def maaae(y_true, y_pred):
    return abs(keras.backend.sum(y_true) - keras.backend.sum(y_pred))
def mssse(y_true, y_pred):
    return (keras.backend.sum(y_true) - keras.backend.sum(y_pred)) * (keras.backend.sum(y_true) - keras.backend.sum(y_pred))
